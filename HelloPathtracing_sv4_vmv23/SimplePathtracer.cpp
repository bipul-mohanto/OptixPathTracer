...
// sanity check: make sure we launch only after first resize is already done:
    ...
    launchParams.samples_per_launch = 8;//8 original, ref 64; // send to device side
    launchParams.frame.offset = make_uint2(0, 0);
    launchParams.frame.redraw = 0;

    // un-used for now (bm) 
    launchParams.viewportSize.x = launchParams.frame.size.x;// 1200;
    launchParams.viewportSize.y = launchParams.frame.size.y;//1024;
...
//! sv   
    ...
    launchParams.frame.fillSize = 4; // sv:4
